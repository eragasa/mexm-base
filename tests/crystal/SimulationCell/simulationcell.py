...
class SimulationCell(object):
    ...
    Attributes:
        ...
        lattice (mexm.crystal.Lattice): the crystal structure of the lattice
    ...
    def H(self, H):
        if isinstance(H, list):
            self.lattice.H = np.array(H)
        elif isinstance(H, np.ndarray):
            self.lattice.H = H.copy()
        else:
            raise TypeError()

    @property
    def a0(self):
        return float(self.lattice.a0)
    ...
    def find_atom_at_position(self, position):
        ...
        if isinstance(position, list):
            ...
            msg = "position must either by a numpy array or a list"
            raise TypeError(msg)

        for k, v in self.atomic_basis.items():
            diff = a.position - position_
            diff_distance = a0 * np.dot(diff, diff)**2
            if diff_distance < self.ptol:
                return k
        return False
    ...
    def add_atom(self, symbol, position, atom_id=None, magmom=0.):
        ...
        if self.atomic_basis is None:
            self.atomic_basis = OrderedDict()

        if not self.find_atom_at_position(position):
            atom_id = "{}{}".format(symbol,
                                    self.get_number_of_atoms(symbol=symbol))
            self.atomic_basis[atom_id] = Atom(atom_id=atom_id,
                                              symbol=symbol,
                                              position=position,
                                              magmom=magmom)
